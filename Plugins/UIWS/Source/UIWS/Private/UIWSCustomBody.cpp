...
void AUIWSCustomBody::CreateMeshSurface()
{
	...
	else
	{
		...
		CustomMeshComp->SetCollisionResponseToChannel(ECC_Visibility, ECR_Block);
		CustomMeshComp->SetCollisionEnabled(ECollisionEnabled::QueryAndPhysics);
	}

	if (AllowCameraUnder == false)
	{
		CustomWaterVolume->SetCollisionResponseToChannel(ECC_Camera, ECR_Block);
	}
...
void AUIWSCustomBody::OnWaterOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor,
