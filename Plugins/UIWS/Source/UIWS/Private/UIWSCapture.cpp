...
AUIWSCapture::AUIWSCapture()
{
	...
	static ConstructorHelpers::FObjectFinder<UMaterialInterface> PP_Depth(
		TEXT("/UIWS/Materials/Capture/PP_UIWSDepthCheck.PP_UIWSDepthCheck"));
...
{
	Super::BeginPlay();
	const FLinearColor ColPos = UKismetMaterialLibrary::GetVectorParameterValue(
		this, MPC_UIWSWaterBodies, TEXT("playerpos"));
	const FVector PlayerLoc = FVector(ColPos.R, ColPos.G, ColPos.B);
...
void AUIWSCapture::MoveCapturePixel()
{
	const FLinearColor ColPos = UKismetMaterialLibrary::GetVectorParameterValue(
		this, MPC_UIWSWaterBodies, TEXT("playerpos"));
	const FVector PlayerLoc = FVector(ColPos.R, ColPos.G, ColPos.B);
	const FVector MoveAmount = SnapPixelWorldSize(PlayerLoc.X, PlayerLoc.Y, SceneCaptureComp->OrthoWidth * (1 / 256));
...
FVector AUIWSCapture::SnapPixelWorldSize(const float PX, const float PY, const float PWS)
{
	FVector VecOut = FVector(FMath::FloorToInt(PX / PWS), FMath::FloorToInt(PY / PWS), 0);
	VecOut = VecOut * PWS;
	return VecOut;
}
...
UTextureRenderTarget2D* AUIWSCapture::SetupCapture(float EdgeTestDepth, const int32 RTRes)
{
	...
	SceneCaptureComp->OrthoWidth = UKismetMaterialLibrary::GetScalarParameterValue(
		this, MPC_UIWSWaterBodies, TEXT("InteractiveDistance"));
