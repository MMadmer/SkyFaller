...
AUIWSManager::AUIWSManager()
{
	...
	static ConstructorHelpers::FObjectFinder<UMaterialInterface> UnderWaterPPGlobal(
		TEXT("/UIWS/Materials/Simulation/PP_UnderWater_Instinstn.PP_UnderWater_Instinstn"));
...
void AUIWSManager::UpdateLightMPCVals()
{
	...
	if (DirectionalLights.Num() > 0)
	{
		...
		if (DirectionalLight)
		{
			...
			UKismetMaterialLibrary::SetScalarParameterValue(this, MPC_UIWSWaterBodies,
			...
			UKismetMaterialLibrary::SetVectorParameterValue(this, MPC_UIWSWaterBodies, TEXT("DirectionalLightVector"),
			                                                FLinearColor(DirectionalLight->GetActorForwardVector()));
		}
	}
	else
	{
		UKismetMaterialLibrary::SetScalarParameterValue(this, MPC_UIWSWaterBodies, TEXT("DirectionalLightIntensity"),
		                                                0);
		UKismetMaterialLibrary::SetVectorParameterValue(this, MPC_UIWSWaterBodies, TEXT("DirectionalLightVector"),
		                                                FLinearColor(FVector(0, 0, 1)));
	}

	UKismetMaterialLibrary::SetScalarParameterValue(this, MPC_UIWSWaterBodies, TEXT("WindChoppyness"), WindTurbulence);
	UKismetMaterialLibrary::SetScalarParameterValue(this, MPC_UIWSWaterBodies, TEXT("CausticBrightness"),
	                                                CausticBrightnessMult);
	UKismetMaterialLibrary::SetVectorParameterValue(this, MPC_UIWSWaterBodies, TEXT("WindVector"),
...
void AUIWSManager::Tick(float DeltaTime)
{
	...
	{
		UKismetMaterialLibrary::SetVectorParameterValue(this, MPC_UIWSWaterBodies, TEXT("playerpos"),
		                                                FLinearColor(PlayerPawn->GetActorLocation()));
	}
	else if (APlayerController* PC = UGameplayStatics::GetPlayerController(this, 0))
	{
		...
		UKismetMaterialLibrary::SetVectorParameterValue(this, MPC_UIWSWaterBodies, TEXT("playerpos"),
		                                                FLinearColor(PC->PlayerCameraManager->GetCameraLocation()));
	}
	else
	{
		...
		UE_LOG(LogTemp, Warning,
		       TEXT(
			       "UIWS Manager failed to find a pawn OR camera to center water simulation on.  UIWSManager.CPP Line 112"
		       ));
	}
...
{
	//clear MPC refs
	for (int i = 0; i < UKismetMaterialLibrary::GetScalarParameterValue(this, MPC_UIWSWaterBodies,
	                                                                    TEXT("SupportedBodyCount")); i++)
	{
		//ckear mpc entries for any stored bodies
		if (i != 0 && i <= UKismetMaterialLibrary::GetScalarParameterValue(
			this, MPC_UIWSWaterBodies, TEXT("SupportedBodyCount")))
		{
			FString ThisBodyPosName = "WaterBody" + FString::FromInt(i) + "_Position";
			//Set my position in param collection
			UKismetMaterialLibrary::SetVectorParameterValue(this, MPC_UIWSWaterBodies, FName(*ThisBodyPosName),
			                                                FLinearColor(FVector(0, 0, -200000)));
