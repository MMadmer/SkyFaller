 ...
 * If using level streaming, only one manager in the persistent level is required.  Non needed in sub-levels.*/
...
public:
	...
	/** This post process volume is used so that the underwater volumes blend correctly in game.
	 *  It shouldn't conflict with other volumes,
	 *  but if you're getting strange results uncheck SpawnPostProcess uproperty on this actor*/
