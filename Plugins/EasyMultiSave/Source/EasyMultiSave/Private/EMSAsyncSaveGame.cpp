...
UEMSAsyncSaveGame::UEMSAsyncSaveGame(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer), bIsActive(0), Mode(), WorldContextObject(nullptr), EMS(nullptr), Data(0),
	  bFinishedStep(0)
{
}

ESaveGameMode UEMSAsyncSaveGame::GetMode(const int32 Data)
{
...
UEMSAsyncSaveGame* UEMSAsyncSaveGame::AsyncSaveActors(UObject* WorldContext, const int32 Data)
{
	if (UEMSObject* EMSObject = UEMSObject::Get(WorldContext))
	{
		if (!EMSObject->IsAsyncSaveOrLoadTaskActive(GetMode(Data)))
		{
			...
			SaveTask->WorldContextObject = WorldContext;
			SaveTask->Data = Data;
