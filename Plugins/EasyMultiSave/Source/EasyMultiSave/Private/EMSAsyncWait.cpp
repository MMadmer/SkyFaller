...
UEMSAsyncWait::UEMSAsyncWait(): Type(), WorldContextObject(nullptr), EMS(nullptr), bSaveOrLoadTaskWasActive(0)
{
}

UEMSAsyncWait* UEMSAsyncWait::AsyncWaitForOperation(UObject* WorldContextObject, const EAsyncCheckType CheckType)
{
...
void UEMSAsyncWait::Activate()
{
	if (EMS)
	{
		...
		if (WaitTime > 0.f)
		{
			...
			EMS->GetTimerManager().SetTimer(TimerHandle,
			                                FTimerDelegate::CreateUObject(this, &UEMSAsyncWait::ForceProceed), WaitTime,
			                                false);
		}
	}
}
