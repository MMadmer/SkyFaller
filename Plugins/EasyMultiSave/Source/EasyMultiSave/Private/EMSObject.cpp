...
void UEMSObject::DeleteCustomSave(const UEMSCustomSaveGame* SaveGame)
{
	if (!SaveGame)
	{
		return;
...
{
	const FString FullSlotSuffix = Underscore + SlotSuffix + SaveType;

	//Gather all save files
	TArray<FString> SaveGameNames;
	IFileManager::Get().FindFiles(SaveGameNames, *AllSaveFiles(), true, false);
	...
	for (const FString& ActualFileName : SaveGameNames)
	{
		...
		if (bIsActualSlot)
		{
			...
			const int Index = ActualFileName.Find(FullSlotSuffix, ESearchCase::IgnoreCase, ESearchDir::FromEnd,
			                                      INDEX_NONE);
...
void UEMSObject::DeleteAllSaveDataForSlot(const FString& SaveGameName)
{
	...
	if (IsConsoleFileSystem())
	{
		...
		IFileManager::Get().FindFiles(SaveGameFiles, *AllSaveFiles(), true, false);
		for (const FString& FileName : SaveGameFiles)
		{
		...
		{
			IFileManager::Get().Delete(*(BaseSaveDir() + FileName), true, false, true);
		}

		bSuccess = !ArrayEmpty(AllFiles);
		if (bSuccess)
		{
...
void UEMSObject::DeleteAllSaveDataForUser(const FString& UserName)
{
	...
	//Try removing folder	
	const bool bSuccess = IFileManager::Get().DeleteDirectory(*UserSaveFile, true, true);
	if (bSuccess)
	{
...
bool UEMSObject::VerifyOrCreateDirectory(const FString& NewDir) const
{
...
static bool SaveBinaryData(const TArray<uint8>& SavedData, const FString& FullSavePath)
{
	ISaveGameSystem* SaveSystem = IPlatformFeaturesModule::Get().GetSaveGameSystem();
	return SaveSystem->SaveGame(false, *FullSavePath, PlayerIndex, SavedData);
}

bool UEMSObject::SaveBinaryArchive(FBufferArchive& BinaryData, const FString& FullSavePath) const
{
	bool bSuccess;
...
bool UEMSObject::UnpackBinaryArchive(const EDataLoadType& LoadType, FMemoryReader FromBinary, UObject* Object)
{
	if (LoadType == EDataLoadType::DATA_Level)
	{
		...
		if (IsNormalMultiLevelSave())
		{
			...
			if (ArrayEmpty(LevelArchiveList))
			{
	...
	else if (LoadType == EDataLoadType::DATA_Player)
	{
		...
		if (IsNormalMultiLevelSave())
		{
			...
			//Copy from disk to memory.
			if (ArrayEmpty(PlayerStackData.LevelPositions))
			{
				PlayerStackData = PlayerStack;
			}

			//Find transform per level
...
void UEMSObject::PrepareLoadAndSaveActors(const uint32& Flags, const EAsyncCheckType& FunctionType,
		...
		if (Actor && IsValidActor(Actor))
		{
			...
			if (FunctionType == EAsyncCheckType::CT_Load)
			{
				if (bFullReload)
				{
					...
					{
						bIsPlayer = (Actor == GetPlayerController()) || (Actor == GetPlayerPawn()) || (Actor ==
							GetPlayerController()->PlayerState);
					}
	...
	for (AActor* Actor : ActorList)
	{
		if (Actor && IsValidForSaving(Actor))
		{
			...
			if (Type == EActorType::AT_Runtime || Type == EActorType::AT_Placed || Type == EActorType::AT_Persistent)
			{
				...
				ActorArray.Type = static_cast<uint8>(Type);
...
void UEMSObject::LoadLevelActors(UEMSAsyncLoadGame* LoadTask)
{
	...
	//Game Mode Actor
	if (!ArrayEmpty(SavedGameMode.Data))
	{
		AGameModeBase* GameMode = GetWorld()->GetAuthGameMode();
	...
	//Game State Actor
	if (!ArrayEmpty(SavedGameState.Data))
	{
		AGameStateBase* GameState = GetWorld()->GetGameState();
...
{
	const EActorType Type = static_cast<EActorType>(ActorArray.Type);
	const bool bRightType = Type == EActorType::AT_Placed || Type == EActorType::AT_Runtime || Type ==
		EActorType::AT_Persistent;
...
EUpdateActorResult UEMSObject::UpdateLevelActor(const FActorSaveData& ActorArray)
{
	...
	for (AActor* Actor : ActorList)
	{
		if (Actor && IsValidActor(Actor))
		{
			...
			{
				//Skips respawn, this should actually never happen since 1.42, since the check comes earlier.
				if (Actor->ActorHasTag(HasLoadedTag))
				{
...
bool UEMSObject::CheckForExistingActor(const FActorSaveData& ActorArray)
{
	...
	if (ThisWorld && ThisWorld->PersistentLevel)
	{
		...
		AActor* NewLevelActor = Cast<AActor>(
			StaticFindObjectFast(nullptr, GetWorld()->PersistentLevel, LoadedActorName));
		if (NewLevelActor)
		{
...
void UEMSObject::SpawnLevelActor(const FActorSaveData& ActorArray)
{
	...
	{
		SpawnClass = Cast<
			UClass>(StaticLoadObject(UClass::StaticClass(), nullptr, *Class, nullptr, LOAD_None, nullptr));
	}
...
void UEMSObject::ProcessLevelActor(AActor* Actor, const FActorSaveData& ActorArray)
{
	...
	if (static_cast<EActorType>(ActorArray.Type) == GetActorType(Actor))
	{
...
void UEMSObject::LoadPlayerActors(UEMSAsyncLoadGame* LoadTask)
{
	...
	//Player State
	if (!ArrayEmpty(SavedPlayerState.Data))
	{
		APlayerState* PlayerState = GetPlayerController()->PlayerState;
				...
				if (ChildActor)
				{
					...
					else
					{
						UE_LOG(LogEasyMultiSave, Warning,
						       TEXT("Child Actor Component has Save Interface, skipping: %s"), *Component->GetName());
					}
				}
			}
			else
			...
			for (const FComponentSaveData& ComponentArray : InComponents)
			{
				if (ComponentArray.Name == BytesFromString(Component->GetName()))
				{
					...
					if (SceneComp)
					{
						...
						{
							SceneComp->SetRelativeTransform(ComponentArray.RelativeTransform, false, nullptr,
...
void UEMSObject::SerializeStructProperties(const UObject* Object)
{
...
void UEMSObject::SerializeMap(const FMapProperty* MapProp)
{
...
void UEMSObject::SerializeArrayStruct(const FArrayProperty* ArrayProp)
{
	FProperty* InnerProperty = ArrayProp->Inner;
	if (InnerProperty)
	{
		//Here we finally get to the struct-property, which hides in the Array->Inner
...
void UEMSObject::SerializeScriptStruct(const UStruct* ScriptStruct)
{
	if (ScriptStruct)
	{
...
{
	return Pawn->ActorHasTag(PlayerPawnAsLevelActorTag);
}
...
{
	return IsMovable(Actor->GetRootComponent()) && !Actor->ActorHasTag(SkipTransformTag) && Actor->
...
bool UEMSObject::IsAsyncSaveOrLoadTaskActive(const ESaveGameMode& Mode, const EAsyncCheckType& CheckType,
...
TSaveGame* UEMSObject::GetDesiredSaveObject(const FString& FullSavePath, const FSoftClassPath& InClassName,
...
UTexture2D* UEMSObject::ImportSaveThumbnail(const FString& SaveGameName) const
{
	const FString SaveThumbnailName = ThumbnailSaveFile(SaveGameName);

	//Suppress warning messages when we don't have a thumb yet.
...
void UEMSObject::ExportSaveThumbnail(UTextureRenderTarget2D* TextureRenderTarget, const FString& SaveGameName) const
{
	const FString SaveThumbnailName = ThumbnailSaveFile(SaveGameName);
	const FText PathError;
	...
	{
		UE_LOG(LogEasyMultiSave, Warning, TEXT("ExportSaveThumbnailRT: Invalid file path provided: %s"),
		       *PathError.ToString());
	}
