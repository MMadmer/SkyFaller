...
public:
	UEMSPluginSettings(): bAdvancedSpawnCheck(0), bSimpleActorNames(0), bAutoSaveStructs(1), bMultiThreadSaving(0),
	                      bPersistentPlayer(0), bPersistentGameMode(0)
	{
	}
	...
	UPROPERTY(config, EditAnywhere, Category = "General Settings",
		meta = (MetaClass = "EMSInfoSaveGame", DisplayName = "Slot Info Class"))
	...
	uint8 bAdvancedSpawnCheck : 1;

	/**
	Actors are saved without their associated Level Name. This can speed up the loading process, but is not suitable for Level Streaming.
	*/
	UPROPERTY(config, EditAnywhere, Category = "Actors", meta = (DisplayName = "Optimized Actor Names"))
	uint8 bSimpleActorNames : 1;
	...
	*/
	...
	UPROPERTY(config, EditAnywhere, AdvancedDisplay, Category = "Save and Load",
		meta = (DisplayName = "Deferred Load Size", EditCondition = "LoadMethod == ELoadMethod::LM_Deferred"))
	int DeferredLoadStackSize = 15;

	/**How long the Async load/wait nodes are allowed to remain fixed in a state.*/
	UPROPERTY(config, EditAnywhere, AdvancedDisplay, Category = "Save and Load",
		meta = (DisplayName = "Async Wait Delay"))
	...
	*/
	...
	UPROPERTY(config, EditAnywhere, Category = "Persistence",
		meta = (DisplayName = "Persistent Player", EditCondition =
			"MultiLevelSaving == EMultiLevelSaveMethod::ML_Disabled"))
	uint8 bPersistentPlayer : 1;

	/**The game mode and game state can be loaded independent of the level.*/
	UPROPERTY(config, EditAnywhere, Category = "Persistence",
		meta = (DisplayName = "Persistent Game Mode", EditCondition =
			"MultiLevelSaving == EMultiLevelSaveMethod::ML_Disabled"))
	uint8 bPersistentGameMode : 1;
